...
bombina.natural<-as.rspec(cbind(bombina.data[1], bombina.data[colnames(bombina.data)%in% indiv.natural$code]))
bombina.light<-as.rspec(cbind(bombina.data[1], bombina.data[colnames(bombina.data)%in% indiv.light$code]))
bombina.dark<-as.rspec(cbind(bombina.data[1], bombina.data[colnames(bombina.data)%in% indiv.dark$code]))

# Visual modeling"
sensdata(visual = "bluetit", achromatic = "none", plot = TRUE, ylab = 'Absorbance')
...
colnames(substr.natural.avg)<-c("wl","natural")
...
colnames(substr.light.avg)<-c("wl","light")
...
colnames(substr.dark.avg)<-c("wl","dark")
...
names(substrate.all.data)<-c("wl",rep("natural",6),rep("light",6), rep("dark",6))
substrate.names<-c(rep("natural",6),rep("light",6), rep("dark",6))
...
vismod.all<- vismodel(all.data, visual = "bluetit", achromatic = "l", illum = flx.illum, relative = FALSE)
contrast.all<-coldist(vismod.all, achro = T)

write.csv(contrast.all,"./data/contrast_all-data.OK.csv", row.names=FALSE)
# Edit this document in Excel to append all metadata information, saved as: "contrast_all-data.EDIT.csv"